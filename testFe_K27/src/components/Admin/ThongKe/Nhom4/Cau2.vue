                    ...
                    <div class="row align-items-center">
                        <div class="col-lg-5">
                            <div class="input-group">
                                <span class="input-group-text fw-bold bg-dark text-white">Từ năm</span>
                        ...
                        <div class="col-lg-5">
                            <div class="input-group">
                                <span class="input-group-text fw-bold bg-dark text-white">Đến năm</span>
                                <input v-model="khoang_nam.den_nam" type="number" min="2000" max="2100" class="form-control"
                                    placeholder="Ví dụ: 2026">
                            </div>
                        </div>
                        <div class="col-lg-2">
                            <button @click="thongKe" class="btn btn-dark text-white w-100 fw-bold">
                                <i class="bx bx-globe"></i> Lọc Biểu Đồ
                            </button>
                        </div>
                    </div>
                </div>
            ...
            <div class="row">
                    ...
                    <div class="card shadow-sm h-100">
                        <div class="card-header bg-dark text-white py-3">
                            <h5 class="mb-0 text-white fw-bold text-uppercase fs-6">
                                <i class="bx bx-flag"></i> Bảng Thống Kê Tuyển Thủ Theo Quốc Tịch
                            </h5>
                        </div>
                                ...
                                <table class="table table-bordered table-hover mb-0 align-middle">
                                    ...
                                    <tbody>
                                        <tr v-for="(value, index) in list_data" :key="index">
                                            ...
                                            <td class="fw-bold text-dark">{{ value.quoc_tich }}</td>
                                            <td class="text-center fw-bold text-primary">{{ value.so_luong_tuyen_thu }} tuyển thủ</td>
                                        </tr>
                ...
                <div class="col-lg-7 mb-3">
                    <div class="card shadow-sm h-100">
                        <div class="card-header bg-dark text-white py-3">
                            <h5 class="mb-0 text-white fw-bold text-uppercase fs-6">
                                <i class="bx bx-bar-chart-square"></i> Biểu Đồ Cột Tuyển Thủ Theo Quốc Tịch
                            </h5>
                        </div>
                        <div class="card-body d-flex justify-content-center align-items-center" style="min-height: 350px;">
                            <div class="w-100" style="height: 320px;">
                                ...
                                <div v-else class="text-center text-muted py-5">
                                    <i class="bx bx-loader-alt bx-spin fs-1 d-block mb-2 text-dark"></i>
...
export default {
    ...
    methods: {
        thongKe() {
            this.is_load = false;
            axios.get('http://127.0.0.1:8000/api/admin/thong-ke/nhom-4/cau-2', { params: this.khoang_nam })
                .then((res) => {
                    this.list_data = res.data.data;
                    this.chartData.labels = res.data.array_quoc_tich;
                    this.chartData.datasets[0].data = res.data.array_so_luong;
                    this.is_load = true;
                })
                .catch(error => {
                    console.error("Lỗi lấy dữ liệu thống kê Nhóm 4 - Câu 2:", error);
                    this.is_load = true;
                });
        }
    }
