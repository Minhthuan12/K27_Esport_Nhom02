                    ...
                    <div class="row align-items-center">
                        <div class="col-lg-5">
                            <div class="input-group">
                                <span class="input-group-text fw-bold bg-primary text-white">Từ năm</span>
                        ...
                        <div class="col-lg-5">
                            <div class="input-group">
                                <span class="input-group-text fw-bold bg-primary text-white">Đến năm</span>
                                <input v-model="khoang_nam.den_nam" type="number" min="2000" max="2100" class="form-control"
                                    placeholder="Ví dụ: 2026">
                            </div>
                        </div>
                        <div class="col-lg-2">
                            <button @click="thongKe" class="btn btn-primary w-100 fw-bold">
                                <i class="bx bx-bar-chart-alt-2"></i> Lọc Biểu Đồ
                            </button>
                        </div>
                    </div>
                </div>
            ...
            <div class="row">
                    ...
                    <div class="card shadow-sm h-100">
                        <div class="card-header bg-primary text-white py-3">
                            <h5 class="mb-0 text-white fw-bold text-uppercase fs-6">
                                <i class="bx bx-list-ul"></i> Bảng Chi Tiết Giải Đấu Theo Năm
                            </h5>
                        </div>
                                ...
                                <table class="table table-bordered table-hover mb-0 align-middle">
                                    ...
                                    <tbody>
                                        <tr v-for="(value, index) in list_data" :key="index">
                                            ...
                                            <td class="text-center fw-semibold">Năm {{ value.nam_to_chuc }}</td>
                                            <td class="text-center fw-bold text-primary">{{ value.so_luong_giai }} giải</td>
                                        </tr>
                ...
                <div class="col-lg-8 mb-3">
                    <div class="card shadow-sm h-100">
                        <div class="card-header bg-primary text-white py-3">
                            <h5 class="mb-0 text-white fw-bold text-uppercase fs-6">
                                <i class="bx bx-bar-chart-alt-2"></i> Biểu Đồ Cột Số Lượng Giải Đấu Theo Năm
                            </h5>
                        </div>
...
export default {
    ...
    methods: {
        thongKe() {
            this.is_load = false;
            axios.get('http://127.0.0.1:8000/api/admin/thong-ke/nhom-1/cau-1', { params: this.khoang_nam })
                .then((res) => {
                    this.list_data = res.data.data;
                    this.chartData.labels = res.data.array_nam;
                    this.chartData.datasets[0].data = res.data.array_so_luong;
                    this.is_load = true;
                })
                .catch(error => {
                    console.error("Lỗi lấy dữ liệu thống kê Nhóm 1 - Câu 1:", error);
                    this.is_load = true;
                });
        }
    }
