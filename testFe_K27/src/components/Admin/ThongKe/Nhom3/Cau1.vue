                    ...
                    <div class="row align-items-center">
                        <div class="col-lg-5">
                            <div class="input-group">
                                <span class="input-group-text fw-bold bg-danger text-white">Từ ngày</span>
                                <input v-model="khoang_ngay.tu_ngay" type="date" class="form-control">
                            </div>
                        </div>
                        <div class="col-lg-5">
                            <div class="input-group">
                                <span class="input-group-text fw-bold bg-danger text-white">Đến ngày</span>
                        ...
                        <div class="col-lg-2">
                            <button @click="thongKe" class="btn btn-danger text-white w-100 fw-bold">
                                <i class="bx bx-trophy"></i> Lọc Biểu Đồ
                            </button>
                        </div>
                    </div>
                </div>
            ...
            <div class="row">
                    ...
                    <div class="card shadow-sm h-100">
                        <div class="card-header bg-danger text-white py-3">
                            <h5 class="mb-0 text-white fw-bold text-uppercase fs-6">
                                <i class="bx bx-medal"></i> Bảng Thống Kê Số Lần Vô Địch
                            </h5>
                        </div>
                                ...
                                <table class="table table-bordered table-hover mb-0 align-middle">
                                    ...
                                    <tbody>
                                        <tr v-for="(value, index) in list_data" :key="index">
                                            ...
                                            <td class="text-center fw-bold text-danger">{{ value.so_lan_vo_dich }} lần 🏆</td>
                                        </tr>
                ...
                <div class="col-lg-7 mb-3">
                    <div class="card shadow-sm h-100">
                        <div class="card-header bg-danger text-white py-3">
                            <h5 class="mb-0 text-white fw-bold text-uppercase fs-6">
                                <i class="bx bx-bar-chart-alt-2"></i> Biểu Đồ Số Cúp Vô Địch Các Đội
                            </h5>
                        </div>
...
export default {
    ...
    methods: {
        thongKe() {
            this.is_load = false;
            axios.get('http://127.0.0.1:8000/api/admin/thong-ke/nhom-3/cau-1', { params: this.khoang_ngay })
                .then((res) => {
                    this.list_data = res.data.data;
                    this.chartData.labels = res.data.array_doi;
                    this.chartData.datasets[0].data = res.data.array_so_lan;
                    this.is_load = true;
                })
                .catch(error => {
                    console.error("Lỗi lấy dữ liệu thống kê Nhóm 3 - Câu 1:", error);
                    this.is_load = true;
                });
        }
    }
